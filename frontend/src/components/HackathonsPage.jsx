...
const HackathonCard = ({ event }) => {
    return (
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden hover:shadow-xl hover:border-blue-300 dark:hover:border-blue-700 transition-all duration-300 flex flex-col h-full group relative">
                    ...
                    {processedEvents.map((monthGroup, idx) => (
                        <div key={idx} className="relative md:pl-24">
                            ...
                            <div className="hidden md:flex absolute left-0 top-0 flex-col items-center">
                                <div className="w-16 h-16 rounded-2xl bg-white dark:bg-slate-800 border-2 border-blue-500 dark:border-blue-400 shadow-lg z-10 flex flex-col items-center justify-center p-2 text-center group">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6">
                                {monthGroup.events.map((event, i) => (
                                    <HackathonCard key={`${event.id}-${i}`} event={event} />
                                ))}
                            </div>
                        </div>
                    ))}
